...
class DTLN_model():
    ...
    def log(self, x):
        '''
        Method for the calculation of log
        '''
        
        # creating frames from the continuous waveform
        logged = tf.math.log(x + 1e-7)
    ...
    def build_DTLN_model(self, norm_stft=False):
        ...
        if norm_stft:
            farend_log_mag = Lambda(self.log)(farend_mag)
            farend_mag_norm = InstantLayerNormalization()(farend_log_mag)
        else:
            # behaviour like in the paper
            farend_mag_norm = farend_mag
        
        if norm_stft:
            nearend_log_mag = Lambda(self.log)(nearend_mag)
            nearend_mag_norm = InstantLayerNormalization()(nearend_log_mag)
        else:
            # behaviour like in the paper
